...
export async function getMusicByUserId(req: Request, res: Response) {
    ...
    try {
        ...
        for(const i of resultsList){
            ...
            ertek={id: i.id, name: i.name, mufaj: i.mufaj}
            if(i.artistId!=null){
...
filter((elem) => elem.toLowerCase().includes(artistInput.toLowerCase()))
*/
...
export async function searchMusicsByArtist(req: Request, res: Response){
    const conn = await config.connection;
    try {
        const [resultsList] = await conn.query("SELECT * FROM musics RIGHT JOIN artists ON artists.id = musics.artistId WHERE artists.name LIKE ?", [`%${req.query.name}%`]);
        if (resultsList.length === 0) {
            res.status(404).json({ message: "No musics found." });
        }
        let ertekek = [];
...
export async function searchMusicsByAlbum(req: Request, res: Response){
    const conn = await config.connection;
    try {
        const [resultsList] = await conn.query("SELECT * FROM musics RIGHT JOIN albums ON albums.id = musics.albumId WHERE albums.name LIKE ?", [`%${req.query.name}%`]);
        if (resultsList.length === 0) {
            res.status(404).json({ message: "No musics found." });
        }
        let ertekek = [];
...
export async function searchMusicsByUsername(req: Request, res: Response){
    const conn = await config.connection;
    try {
        const [resultsList] = await conn.query("SELECT * FROM musics RIGHT JOIN users ON users.id = musics.uploaderId WHERE users.username LIKE ?", [`%${req.query.name}%`]);
        if (resultsList.length === 0) {
            res.status(404).json({ message: "No musics found." });
        }
        let ertekek = [];
...
export async function searchPlaylistsByUsername(req: Request, res: Response){
    const conn = await config.connection;
    try {
        const [results] = await conn.query("SELECT * FROM playlists RIGHT JOIN users ON users.id = playlists.ownerId WHERE users.username LIKE ?", [`%${req.query.name}%`]);
        res.setHeader('Cache-Control', 'no-store');
        if (results.length == 0){
            res.status(300).json({ message: "No playlists found for this user." });
            return;
